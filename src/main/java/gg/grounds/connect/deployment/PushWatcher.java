...
public final class PushWatcher {
    ...
    public PushWatcher(ClientTaskRunner runner, AuthenticatedApi api, AuthService auth, ProjectService projects) {
        this.runner = runner;
        this.api = api;
        this.auth = auth;
    ...
    public void watchInFlightPushes(String projectId, PushStatusSink sink) {
        runner.execute(() -> {
            try {
                List<Push> inflight = api.withAuthRetry(token -> api.api().listInFlightPushes(token, projectId));
                for (Push p : inflight) {
    ...
    private void streamPush(Push push, PushStatusSink sink) {
        runner.execute(() -> {
            try {
                ...
                api.api().streamSse(token, "/v1/pushes/" + push.id() + "/logs", (event, data) -> {
