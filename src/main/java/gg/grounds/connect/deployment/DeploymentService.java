...
public final class DeploymentService {
    ...
    public void fetchRollbackTargets(String name, String projectId, AsyncCallback<List<RollbackTarget>> cb) {
        runner.execute(() -> {
            try {
    ...
    public void retryLatestBuild(String name, String projectId, Runnable onStarted, Consumer<String> onError) {
        runner.execute(() -> {
            try {
                Push latest = api.withAuthRetry(token -> api.api().getLatestPush(token, projectId, name));
                if (latest == null) {
                ...
                if (!"build_failed".equals(st) && !"deploy_failed".equals(st)) {
                    runner.onClient(() -> onError.accept("latest build is '" + st + "' — nothing to retry"));
                    return;
                }
                api.withAuthRetry(token -> {
