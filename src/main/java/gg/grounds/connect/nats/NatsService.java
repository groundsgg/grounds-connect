...
public final class NatsService {
    ...
    public void fetchCluster(String projectId, AsyncCallback<Nats.Snapshot> cb) {
        runner.execute(() -> {
            try {
                Nats.Snapshot snap = api.withAuthRetry(token -> api.api().getClusterNats(token, projectId));
    ...
    /**
     * Tails the workspace NATS broker via {@code GET /v1/cluster/nats/tail} (SSE).
     * Runs until the stream ends or {@code cancelled} returns true.
     */
    public void tail(String projectId, List<String> subjects, NatsTailSink sink, BooleanSupplier cancelled) {
        runner.execute(() -> {
            try {
                ...
                api.api().streamSse(token, path.toString(), (event, data) -> {
                    switch (event) {
                        case "message" -> {
                        ...
                        case "error" -> runner.onClient(() -> sink.onInfo("== error: " + jsonField(data, "reason") + " =="));
                        case "warning" -> runner.onClient(() -> sink.onInfo("== warning: " + jsonField(data, "reason") + " =="));
                        default -> { }
                    }
                }, cancelled);
