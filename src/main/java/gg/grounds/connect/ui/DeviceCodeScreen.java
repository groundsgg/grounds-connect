  ...
  protected void init() {
    ...
    addRenderableWidget(
        new StringWidget(
            ...
            300,
            12,
            Component.translatable("grounds_connect.device.instructions"),
            this.font));

    if (code != null) {
      addRenderableWidget(
          new StringWidget(
              cx - 150, y + 44, 300, 12, Component.literal(code.verificationUri()), this.font));
      addRenderableWidget(
          new StringWidget(
              cx - 150,
              y + 62,
              300,
              12,
              Component.translatable("grounds_connect.device.code", code.userCode()),
              this.font));
    }
    ...
    if (code != null) {
      addRenderableWidget(
          Button.builder(
                  Component.translatable("grounds_connect.device.open"),
                  b -> openVerificationPage())
              .bounds(cx - 154, by, 100, 20)
              .build());
      addRenderableWidget(
          Button.builder(
                  Component.translatable("grounds_connect.device.copyCode"),
                  b -> copy(code.userCode()))
              .bounds(cx - 50, by, 100, 20)
              .build());
      addRenderableWidget(
          Button.builder(
                  Component.translatable("grounds_connect.device.copyUrl"),
                  b -> copy(code.verificationUriComplete()))
              .bounds(cx + 54, by, 100, 20)
              .build());
    }
    addRenderableWidget(
        Button.builder(Component.translatable("grounds_connect.device.cancel"), b -> onClose())
            .bounds(cx - 50, by + 24, 100, 20)
            .build());
