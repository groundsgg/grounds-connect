...
public final class ServerListModel {
  ...
  public void replaceServers(List<GroundsServer> servers) {
    ...
    for (GroundsServer server : servers) {
      ...
      ServerEntry entry =
          new ServerEntry(server.name(), server.address(), data, server.state(), server.type());
      built.add(entry);
  ...
  /**
   * Flattens the tree into the visible list: roots (pins first), with each expanded proxy's
   * backends after it.
   */
  public List<ServerEntry> visibleEntries(GroundsConfig cfg) {
    ...
    roots.sort(
        Comparator.comparingInt((ServerEntry e) -> e.favorite ? 0 : 1)
   ...
   * servers nest under the project's proxy (single-proxy-per-project assumption -- see notes). With
   * no proxy in the project, backends stay top-level joinable (e.g. a standalone gamemode).
   */
  private List<ServerEntry> buildTree(
      List<ServerEntry> proxies, List<ServerEntry> backends, List<ServerEntry> standalone) {
