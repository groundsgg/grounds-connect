...
public final class GroundsServersScreen extends Screen {
    ...
    public GroundsServersScreen(Screen lastScreen) {
        ...
        this.actions = new ServerScreenActions(this, services,
                () -> serverList == null ? null : serverList.selected(), () -> currentProjectId);
    }
    ...
    protected void init() {
        ...
        addRenderableWidget(Button.builder(Component.translatable("grounds_connect.action.retry"), b -> actions.retryBuild())
                .bounds(this.width / 2 - 153, recoveryY, 100, 20).build());
        addRenderableWidget(Button.builder(Component.translatable("grounds_connect.action.rollback"), b -> actions.rollbackSelected())
                .bounds(this.width / 2 - 50, recoveryY, 100, 20).build());
        addRenderableWidget(Button.builder(Component.literal("NATS"), b -> actions.openNats())
                .bounds(this.width / 2 + 53, recoveryY, 100, 20).build());

        int by = this.height - 28;
        addRenderableWidget(Button.builder(Component.translatable("grounds_connect.action.join"), b -> actions.joinSelected())
                .bounds(this.width / 2 - 153, by, 100, 20).build());
        addRenderableWidget(Button.builder(Component.translatable("grounds_connect.action.logs"), b -> actions.openLogs())
    ...
    private final class LoaderListener implements ServerDataLoader.Listener {
        ...
        public boolean onServersLoaded(String projectId, List<GroundsServer> servers) {
            ...
            setStatusMessage(model.entries().isEmpty() ? Component.translatable("grounds_connect.status.noServers") : null);
            return true;
        }
