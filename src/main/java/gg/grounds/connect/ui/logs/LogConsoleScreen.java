...
/**
 * Auto-tailing log console with a Runtime/Build toggle. Runtime tails {@code
 * /v1/deployments/:name/logs}; Build resolves the deployment's latest push and tails {@code
 * /v1/pushes/:id/logs}. Copy to clipboard, or Share to mclo.gs.
 */
public final class LogConsoleScreen extends Screen {
  ...
  protected void init() {
    ...
    runtimeBtn =
        Button.builder(
                Component.translatable("grounds_connect.logs.runtime"),
                b -> setSource(Source.RUNTIME))
            .bounds(this.width / 2 - 102, 22, 100, 20)
            .build();
    buildBtn =
        Button.builder(
                Component.translatable("grounds_connect.logs.build"), b -> setSource(Source.BUILD))
            .bounds(this.width / 2 + 2, 22, 100, 20)
            .build();
    ...
    list =
        new LogLineList(this.minecraft, this.width, listHeight, listTop, this.font.lineHeight + 2);
    addRenderableWidget(list);

    shareStatus =
        new StringWidget(
            this.width / 2 - 150, this.height - 42, 300, 12, Component.empty(), this.font);
    addRenderableWidget(shareStatus);

    int by = this.height - 28;
    addRenderableWidget(
        Button.builder(Component.translatable("grounds_connect.logs.copy"), b -> copyAll())
            .bounds(this.width / 2 - 153, by, 100, 20)
            .build());
    addRenderableWidget(
        Button.builder(Component.translatable("grounds_connect.logs.share"), b -> shareLogs())
          ...
          .fetchLatestPush(
              ...
              new AsyncCallback<>() {
                ...
                public void onResult(Push push) {
                  ...
                  Grounds.services().logs().stream(
                      "/v1/pushes/" + enc(push.id()) + "/logs",
                      LogConsoleScreen.this::onLine,
                      flag::get);
                }
                ...
                public void onError(Throwable error) {
                  addLine(
                      Component.translatable("grounds_connect.logs.error", error.getMessage())
                          .getString());
                }
              });
    }
