 ...
 * connections and JetStream streams. Requires an active dev workspace for the project. A "Live
 * tail" button opens {@link NatsTailScreen} for the live message stream.
 */
public final class NatsScreen extends Screen {
  ...
  protected void init() {
    ...
    addRenderableWidget(
        Button.builder(Component.translatable("grounds_connect.control.refresh"), b -> load())
            .bounds(this.width / 2 - 102, 22, 100, 20)
            .build());
    addRenderableWidget(
        Button.builder(Component.translatable("grounds_connect.nats.liveTail"), b -> openTail())
  ...
  private void render(Nats.Snapshot s) {
    ...
    if (b == null) {
      ...
      addLine(
          I18n.get(
              "grounds_connect.nats.broker.stats",
              b.connections(),
              b.inMsgs(),
              human(b.inBytes()),
              b.outMsgs(),
    ...
    if (js == null || js.streams() == 0) {
      ...
      addLine(
          I18n.get(
              "grounds_connect.nats.jetstream.stats",
              js.streams(),
              js.messages(),
              human(js.bytes())));
      for (Nats.Stream st : js.list()) {
        addLine(
            I18n.get(
                "grounds_connect.nats.jetstream.stream",
                st.name(),
                String.join(", ", st.subjects()),
                st.messages(),
                st.consumers()));
      }
    }
