...
/**
 * Live NATS message tail ({@code GET /v1/cluster/nats/tail}, SSE). A subject filter (default {@code
 * >}, up to 16 subjects, comma/space separated) plus Pause and Clear. Requires an active dev
...
public final class NatsTailScreen extends Screen {
  ...
  protected void init() {
    ...
    addRenderableWidget(
        Button.builder(Component.translatable("grounds_connect.nats.apply"), b -> restart())
            .bounds(this.width / 2 + 76, 22, 74, 20)
            .build());
    ...
    pauseBtn =
        Button.builder(Component.translatable("grounds_connect.nats.pause"), b -> togglePause())
            .bounds(this.width / 2 - 153, this.height - 28, 100, 20)
            .build();
    addRenderableWidget(pauseBtn);
    addRenderableWidget(
        Button.builder(
                Component.translatable("grounds_connect.nats.clear"),
                b -> {
                  if (list != null) {
                    list.clear();
                  }
    ...
    addRenderableWidget(
        Button.builder(CommonComponents.GUI_BACK, b -> onClose())
            .bounds(this.width / 2 + 53, this.height - 28, 100, 20)
            .build());
  ...
  private void restart() {
    ...
    addLine(
        Component.translatable("grounds_connect.nats.tailing", String.join(",", subs)).getString());
    Grounds.services()
        .nats()
        .tail(
            projectId,
  ...
  private void togglePause() {
    ...
    pauseBtn.setMessage(
        Component.translatable(
            paused ? "grounds_connect.nats.resume" : "grounds_connect.nats.pause"));
  }
