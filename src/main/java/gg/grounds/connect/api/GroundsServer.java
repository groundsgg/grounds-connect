...
/**
 * A connectable Minecraft server derived from a forge deployment whose {@code publicUrl} uses the
 * {@code minecraft://} scheme.
 *
 ...
 * @param type forge deployment type. The API returns the normalized internal form ({@code
 *     plugin-velocity} / {@code plugin-paper} / {@code gamemode} / {@code service}); we also accept
 *     the short public/manifest forms ({@code velocity} / {@code paper} / {@code gamemode}) to be
 *     vocabulary-proof.
 */
public record GroundsServer(String name, String address, String state, String type) {
  ...
  /**
   * A backend game server (Paper, Agones gamemode or Minestom) — joined via the proxy, not
   * directly.
   */
  public boolean isBackend() {
    return "gamemode".equals(type)
        || "plugin-paper".equals(type)
        || "paper".equals(type)
