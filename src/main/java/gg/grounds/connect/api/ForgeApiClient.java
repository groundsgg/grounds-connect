...
public final class ForgeApiClient {
  ...
  public boolean isReady() {
    try {
      HttpRequest req =
          HttpRequest.newBuilder(URI.create(baseUrl + "/readyz"))
  ...
  public List<Project> listProjects(String accessToken) throws Exception {
    ...
    for (JsonElement el : itemsOf(body)) {
      ...
      out.add(new Project(str(o, "id"), str(o, "slug"), str(o, "name"), str(o, "role")));
    }
    ...
    Constants.LOG.debug(
        "[{}] /v1/projects -> {} project(s):{}", Constants.MOD_NAME, out.size(), names);
    return out;
  }

  /**
   * Lists a project's Minecraft servers — deployments with a {@code minecraft://} publicUrl (Paper
   * / Velocity / gamemode) plus {@code minestom} deployments, which forge renders as a {@code
   * service} with an {@code https://} URL so they can only be identified via {@code /v1/apps}
   * (their manifest {@code publicType}). Skips soft-deleted rows; follows pagination.
   */
  public List<GroundsServer> listMinecraftServers(String accessToken, String projectId)
      throws Exception {
    ...
    for (int page = 0; page < MAX_DEPLOYMENT_PAGES; page++) {
      StringBuilder path =
          new StringBuilder("/v1/deployments?limit=100&projectId=")
    ...
    Constants.LOG.debug(
        "[{}] project {}: {} deployment(s) total, {} minecraft server(s):{}",
        Constants.MOD_NAME,
        projectId,
        total,
        out.size(),
        raw.length() == 0 ? " (none)" : raw);
    return out;
  }
  ...
  private Map<String, String> fetchPublicTypes(String accessToken, String projectId) {
    ...
    try {
      ...
      Constants.LOG.debug(
          "[{}] could not read /v1/apps publicTypes: {}", Constants.MOD_NAME, e.toString());
    }
    return map;
  }

  /**
   * The flavor's manifest {@code publicType}, falling back to its flavorKey (== publicType when
   * inferred).
   */
  ...
  /**
   * Strips scheme + path from a URL, leaving {@code host[:port]} — for displaying non-minecraft
   * backends.
   */
  ...
  public List<Push> listInFlightPushes(String accessToken, String projectId) throws Exception {
    JsonObject body =
        getJson(
            "/v1/pushes?statusGroup=in_flight&limit=20&projectId=" + enc(projectId), accessToken);
  ...
  public void resume(String accessToken, String name, String projectId) throws Exception {
    mutate(
        accessToken,
        "POST",
        "/v1/deployments/" + enc(name) + "/resume?projectId=" + enc(projectId),
        null);
  }
  ...
  public void retryPush(String accessToken, String pushId, String projectId) throws Exception {
    mutate(
        accessToken,
        "POST",
        "/v1/pushes/" + enc(pushId) + "/retry?projectId=" + enc(projectId),
        null);
  }
   ...
   */
  public Push getLatestPush(String accessToken, String projectId, String deploymentName)
      throws Exception {
    JsonObject body = getJson("/v1/apps?projectId=" + enc(projectId), accessToken);
    for (JsonElement appEl : itemsOf(body)) {
  ...
  public String getAppName(String accessToken, String projectId, String deploymentName)
      throws Exception {
    JsonObject body = getJson("/v1/apps?projectId=" + enc(projectId), accessToken);
    for (JsonElement appEl : itemsOf(body)) {
      JsonObject app = appEl.getAsJsonObject();
  ...
  public List<RollbackTarget> listRollbackTargets(
      String accessToken, String projectId, String appName) throws Exception {
    JsonObject body =
        getJson(
            "/v1/pushes?app="
                + enc(appName)
                + "&target=dev&statusGroup=ready&limit=20&projectId="
                + enc(projectId),
            accessToken);
  ...
  public void rollback(String accessToken, String name, String projectId, String pushId)
      throws Exception {
    mutate(
        accessToken,
        "POST",
        "/v1/deployments/" + enc(name) + "/rollback?projectId=" + enc(projectId),
        "{\"pushId\":\"" + pushId + "\"}");
  }

  /** {@code GET /v1/deployments/:name/runtime} -> live replicas + devCluster state. */
  public DeploymentRuntime getRuntime(String accessToken, String name, String projectId)
      throws Exception {
    JsonObject o =
        getJson(
            "/v1/deployments/" + enc(name) + "/runtime?projectId=" + enc(projectId), accessToken);
    int ready = 0;
    int desired = 0;
  ...
  public Nats.Snapshot getClusterNats(String accessToken, String projectId) throws Exception {
    ...
    if (o.has("broker") && o.get("broker").isJsonObject()) {
      ...
      broker =
          new Nats.Broker(
              ...
              longOr(b, "inBytes"),
              longOr(b, "outBytes"),
              intOr(b, "subscriptions"),
              intOr(b, "slowConsumers"));
    }
      ...
      for (JsonElement el : o.getAsJsonArray("connections")) {
        ...
        connections.add(
            new Nats.Conn(
                longOr(c, "cid"),
                str(c, "name"),
                stringList(c, "subjects"),
                c.has("system") && c.get("system").getAsBoolean()));
      }
    }
    ...
    if (o.has("jetstream") && o.get("jetstream").isJsonObject()) {
      ...
      jetstream =
          new Nats.JetStream(
              intOr(j, "streams"), longOr(j, "messages"), longOr(j, "bytes"), streams);
    }
  ...
  /**
   * Consumes a {@code text/event-stream} endpoint, invoking {@code handler} per SSE frame until the
   * stream ends or {@code cancelled} returns true. Blocking — call on a background thread.
   */
