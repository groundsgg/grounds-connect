...
/**
 * Stored OIDC credentials. {@code expiresAt}/{@code refreshExpiresAt} are epoch seconds (0 =
 * unknown / never). The Grounds refresh token is an offline token with no expiry, so in practice
 * only {@code expiresAt} matters.
 */
