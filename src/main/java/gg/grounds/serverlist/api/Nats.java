...
public final class Nats {
    ...
    /** Snapshot from {@code GET /v1/cluster/nats}. {@code broker}/{@code jetstream} are null if unreachable. */
    public record Snapshot(Broker broker, List<Event> events, List<Conn> connections, JetStream jetstream) {}

    public record Broker(int connections, long inMsgs, long outMsgs, long inBytes, long outBytes,
                         int subscriptions, int slowConsumers) {}
    ...
    public record Stream(String name, List<String> subjects, long messages, long bytes,
                         int consumers, long firstSeq, long lastSeq) {}

    /** A live message from {@code GET /v1/cluster/nats/tail} ({@code data} is UTF-8 or {@code base64:…}). */
