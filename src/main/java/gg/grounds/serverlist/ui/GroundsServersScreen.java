...
import gg.grounds.serverlist.api.DeploymentRuntime;
import gg.grounds.serverlist.api.GroundsServer;
import gg.grounds.serverlist.api.Project;
import gg.grounds.serverlist.config.GroundsConfig;
import gg.grounds.serverlist.ui.GroundsServerList.ServerEntry;
...
public final class GroundsServersScreen extends Screen {
    ...
    public GroundsServersScreen(Screen lastScreen) {
        super(Component.literal("Grounds DevMod"));
    ...
    protected void init() {
        ...
        addRenderableWidget(Button.builder(Component.translatable("grounds.control.refresh"), b -> reloadServers())
                .bounds(192, row, 80, 20).build());
        addRenderableWidget(Button.builder(Component.translatable("grounds.menu.logout"), b -> logout())
                .bounds(this.width - 104, row, 100, 20).build());

        if (platformReady) {
            search = new EditBox(this.font, this.width / 2 - 150, searchY, 300, 18,
                    Component.translatable("grounds.search.hint"));
            search.setHint(Component.translatable("grounds.search.hint"));
            ...
            StringWidget banner = new StringWidget(this.width / 2 - 150, searchY + 4, 300, 12,
                    Component.translatable("grounds.platform.unreachable")
        ...
        addRenderableWidget(Button.builder(Component.translatable("grounds.action.join"), b -> joinSelected())
        ...
        if (!initialLoadDone) {
            ...
            if (session.isLoggedIn()) {
                if (session.cachedProjects().isEmpty() && !projectsRequested) {
                    ...
                    setStatus(Component.translatable("grounds.status.loadingProjects"));
                    loadProjects();
                } else {
                    reloadServers();
                }
            } else {
                setStatus(Component.translatable("grounds.control.login"));
            }
        }
    }
    ...
    private AbstractWidget buildProjectButton(int y) {
        ...
        if (selected != null && !session.cachedProjects().isEmpty()) {
            return CycleButton.builder((Project p) -> Component.literal(p.displayName()), selected)
                    ...
                    .create(8, y, 180, 20, Component.translatable("grounds.control.project.prefix"),
                            (btn, value) -> onProjectChosen(value));
        }
        Button placeholder = Button.builder(
                        Component.translatable(session.isLoggedIn()
                                ? "grounds.control.project.loading" : "grounds.control.project.placeholder"),
                        b -> {})
    ...
    private void loadProjects() {
        session.fetchProjects(new GroundsSession.Callback<>() {
            ...
            public void onError(Throwable error) {
                if (isCurrent()) {
                    setStatus(Component.translatable("grounds.error.projects", msg(error)));
                }
            }
        });
    }
    ...
    private void reloadServers() {
        ...
        if (selected == null) {
            ...
            applyView();
            setStatus(Component.translatable("grounds.status.noProjects"));
            return;
        }
        setStatus(Component.translatable("grounds.status.loadingServers"));
        ...
        session.fetchServers(projectId, new GroundsSession.Callback<>() {
            ...
            public void onResult(List<GroundsServer> value) {
                ...
                applyView();
                pingAll(built);
                setStatus(built.isEmpty() ? Component.translatable("grounds.status.noServers") : null);
            ...
            public void onError(Throwable error) {
                if (isCurrent()) {
                    setStatus(Component.translatable("grounds.error.servers", msg(error)));
                }
            }
        });
    }
    ...
    private void joinSelected() {
        ...
        if (entry.depth > 0 || entry.isMinestom()) { // backend / minestom — only reachable through the proxy
            setStatus(Component.translatable("grounds.status.joinViaProxy"));
            return;
        }
    ...
    private void wakeThenConnect(ServerEntry entry) {
        ...
        session.resumeAndAwait(entry.name, currentProjectId,
                ...
                error -> {
                    if (isCurrent()) {
                        setStatus(Component.translatable("grounds.error.servers", error));
                    }
                });
    }
