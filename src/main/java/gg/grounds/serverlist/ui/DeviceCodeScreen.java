...
import gg.grounds.serverlist.auth.KeycloakClient;
...
public final class DeviceCodeScreen extends Screen {
    ...
    private Component status = Component.translatable("grounds.device.requesting");
    ...
    public DeviceCodeScreen(Screen parent, Runnable onComplete) {
        super(Component.translatable("grounds.device.title"));
        this.parent = parent;
    ...
    protected void init() {
        ...
        addRenderableWidget(new StringWidget(cx - 150, y + 22, 300, 12,
                Component.translatable("grounds.device.instructions"), this.font));
        ...
        if (code != null) {
            ...
            addRenderableWidget(new StringWidget(cx - 150, y + 62, 300, 12,
                    Component.translatable("grounds.device.code", code.userCode()), this.font));
        }
        ...
        int by = y + 116;
        if (code != null) {
            addRenderableWidget(Button.builder(Component.translatable("grounds.device.open"),
                            b -> openVerificationPage())
                    .bounds(cx - 154, by, 100, 20).build());
            addRenderableWidget(Button.builder(Component.translatable("grounds.device.copyCode"),
                            b -> copy(code.userCode()))
                    .bounds(cx - 50, by, 100, 20).build());
            addRenderableWidget(Button.builder(Component.translatable("grounds.device.copyUrl"),
                            b -> copy(code.verificationUriComplete()))
                    .bounds(cx + 54, by, 100, 20).build());
        }
        addRenderableWidget(Button.builder(Component.translatable("grounds.device.cancel"), b -> onClose())
        ...
        if (!started) {
            ...
            handle = GroundsSession.get().startLogin(new GroundsSession.LoginListener() {
                ...
                public void onCode(KeycloakClient.DeviceCode c) {
                    code = c;
                    status = Component.translatable("grounds.device.waiting");
                    rebuildWidgets();
                }
                ...
                public void onError(String message) {
                    status = Component.translatable("grounds.error.login", message);
                    rebuildWidgets();
                }
            });
        }
    ...
    private void copy(String text) {
        if (text != null && minecraft != null) {
            ...
            status = Component.translatable("grounds.device.copied");
            rebuildWidgets();
        }
    }
